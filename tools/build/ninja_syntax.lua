...
local strmatch, strfind, strformat = _s.match, _s.find, _s.format
local tinsert, tconcat, tlen = _t.insert, _t.concat, _t.maxn or _len
local unpack, pack = _G.unpack or _t.unpack, function(...)
  return {...}
end

---@alias Scalar string|number|boolean
---@alias ListInput Scalar|Scalar[]
---@alias VariableValue Scalar|Scalar[]
---@alias VariablePair [string, VariableValue]
---@alias BuildVariables table<string, VariableValue>|VariablePair[]
---@alias ExpansionVariables table<string, string|number>

---@class OutputType
---@field write fun(self: OutputType, ...: string)
---@field flush fun(self: OutputType)
---@field close fun(self: OutputType)

---@class RuleOptions
---@field description? VariableValue
---@field deps? VariableValue
---@field depfile? VariableValue
---@field pool? VariableValue
---@field rspfile? VariableValue
---@field rspfile_content? VariableValue
---@field generator? boolean
---@field restat? boolean

---@class ninja_syntax
---@field DEFAULT_LINE_WIDTH integer
---@field DEFAULT_BLANK_LINES integer
---@field Writer Writer
...
-- LuaJIT's minilua also doesn't have a tostring function.
...
if not tostring then
  tostring = function(value)
    ...
    if valtype == 'string' then
      ...
      return strformat("%s", value) -- Apparently works for everything
    end
  end
end
...
---@param vars? ExpansionVariables searched if var is not found in locals
---@param locals? ExpansionVariables first place searched for variables
...
function ninja.expand(str, vars, locals)
  ...
  local result = str:gsub('%$([%w_]*)', expand):gsub('%$([%$:%s])', '%1')
  return result
end
...
---@return string[] strings table input converted to a table of strings
...
---@param word string text to escape
---@return string escaped escaped version of word
---@return integer substitutions
---@see ninja_syntax.escape
...
do
  ...
  local function wrapafter(text, index)
    local next_index = strfind(text, '%s+', index)
    if next_index == nil then return 0 end
    if iseven(count_dollars_before_index(text, next_index)) then
      return next_index
    else
      return wrapafter(text, next_index+1)
    end
  end
  ...
  ---@alias OutputParam string|file*|OutputType

  --- Writer Implementation
  ---@class Writer
  ---@field width integer
  ---@field blanklines integer
  ---@field output file*|OutputType|nil
  ---@field private _P { lineblank: boolean, blankscount: integer }
  ---@overload fun(output: OutputParam, width?: integer, blanklines?: integer): Writer
  local Writer = {}
...
---@diagnostic disable-next-line: param-type-mismatch
  ...
  --- Writer constructor
  ---@param output OutputParam output filepath or file handle
  ---@param width? integer maximum line width for wrapping (default=78)
  ---@param blanklines? integer maximum consecutive blank lines (default=1)
  ---@return Writer writer
  function Writer.new(output, width, blanklines)
    local self = setmetatable({}, Writer)---@diagnostic disable-line: param-type-mismatch
  ...
  --- Outputs a newline.
  -- Blank lines are limited to the writer's blanklines field if non-zero.
  ---@return Writer self
  ...
  ---@param value? VariableValue variable value
  ---@param indent? integer indent level (default=0)
  ---@return Writer self
  ...
  ---@param kwargs? RuleOptions additional optional rule configuration
  ---@return Writer self
  ...
  ---@param outputs ListInput output file(s)/target(s)
  ---@param rule string rule to use
  ---@param inputs ListInput input file(s)/target(s)
  ---@param implicit? ListInput implicit dependencies
  ---@param order_only? ListInput force build to occur after target(s)
  ---@param variables? BuildVariables any shadowed variables
  ---@param implicit_outputs? ListInput no clue
  ---@return Writer self
  function Writer:build(outputs, rule, inputs, implicit, order_only,
    ...
    if order_only ~= nil then
      tinsert(all_inputs, '||')
      extend(all_inputs, map(ninja.escape_path, ninja.as_list(order_only), {}))
    end
  ...
  ---@param raw? boolean raw controls whether to escape path or leave it raw
  ---@return Writer self
  ...
  ---@param raw? boolean controls whether to escape path or leave it raw (default=false)
  ---@return Writer self
  ...
  ---@return integer count the count of '$' characters
  ...
  ninja.Writer = Writer ---@diagnostic disable-line: assign-type-mismatch
end
