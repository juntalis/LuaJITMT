...
#define VSTACK_CONST		0x10	/* Constant variable. */
...
static void gola_resolve(LexState *ls, FuncScope *bl, MSize idx)
{
  ...
  for (; vg < vl; vg++)
	...
	if (gcrefp(vg->name, GCstr) == NAME_CONT) {
...
static void proto_finish(LexState *ls, ExpDesc *e, ptrdiff_t oldbase)
{
  ...
  uint32_t anchoridx;
  GCproto *pt = fs_finish(ls, (ls->lastline = ls->linenumber), &anchoridx);
  FuncState *pfs = ls->fs;
